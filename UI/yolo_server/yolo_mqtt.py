...
os.environ["OPENVINO_EXECUTION_MODE"] = "THROUGHPUT"
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        t2 = time.time()
        client.publish("camera/results", json.dumps(result_message), qos=0, retain=False)
        t3 = time.time()
        print(
            f"decode={int((t1-t0)*1000)}ms  infer={int((t4-t1)*1000)}ms  cv2={int((t2-t4)*1000)}  publish={int((t3-t2)*1000)}ms"
        )
